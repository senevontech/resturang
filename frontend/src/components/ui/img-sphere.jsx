...
// src/components/ui/img-sphere.jsx
import React, { useState, useEffect, useRef, useCallback, useMemo } from "react";
import { X } from "lucide-react";

/**
 * SphereImageGrid - Interactive 3D Image Sphere Component (JS version)
 */
...
export default function SphereImageGrid({
  ...
  // ✅ MOBILE BOOST (makes sphere bigger on small screens)
  const isMobile = containerSize <= 420;
  const mobileBoost = isMobile ? 1.35 : 1; // bigger images
  const radiusBoost = isMobile ? 1.12 : 1; // slightly bigger sphere

  const actualSphereRadius = (sphereRadius || containerSize * 0.5) * radiusBoost;
  const baseImageSize = containerSize * baseImageScale * mobileBoost;
  ...
  const calculateWorldPositions = useCallback(() => {
    ...
    for (let i = 0; i < adjustedPositions.length; i++) {
      ...
      for (let j = 0; j < adjustedPositions.length; j++) {
        ...
        const minDistance = (imageSize + otherSize) / 2 + (isMobile ? 18 : 25);

        if (distance < minDistance && distance > 0) {
          const overlap = minDistance - distance;
          const reductionFactor = Math.max(0.45, 1 - (overlap / minDistance) * 0.6);
          adjustedScale = Math.min(adjustedScale, adjustedScale * reductionFactor);
        }
      }

      adjustedPositions[i] = { ...pos, scale: Math.max(isMobile ? 0.3 : 0.25, adjustedScale) };
    }

    return adjustedPositions;
  }, [imagePositions, rotation, actualSphereRadius, baseImageSize, isMobile]);
  ...
  const renderImageNode = useCallback(
    (image, index) => {
      ...
      // ✅ slightly stronger hover on mobile so it feels more interactive
      const hoverMul = isMobile ? 1.15 : hoverScale;
      const finalScale = isHovered ? Math.min(1.25, hoverMul / position.scale) : 1;
      ...
      return (
        ...
        >
          <div className="relative h-full w-full overflow-hidden rounded-full border-2 border-white/60 shadow-lg">
            <img
              src={image.src}
              alt={image.alt}
    ...
    [worldPositions, baseImageSize, containerSize, hoveredIndex, isMobile, hoverScale]
  );
  ...
  if (!isMounted) {
    return (
      <div className="flex items-center justify-center rounded-lg bg-gray-100 animate-pulse" style={{ width: containerSize, height: containerSize }}>
  ...
  return (
    <>
      ...
      {/* ✅ Background set to #dcddd7 + subtle vignette */}
      <div
      ...
      >
        <div
          ref={containerRef}
          className="relative h-full w-full select-none cursor-grab active:cursor-grabbing"
          style={{
